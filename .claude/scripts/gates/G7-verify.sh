...
# G7: Security verification gate
if [ -f ".claude/scripts/lib/project-config.sh" ]; then
  source ".claude/scripts/lib/project-config.sh"
  cmd="$(command_for_gate "security" 2>/dev/null || true)"
  if [ -n "$cmd" ] && [ "$cmd" != "N/A" ] && ! [[ "$cmd" == echo* ]]; then
    echo "[RUN] G7 Security: $cmd"
    bash -lc "$cmd" || true
  fi
fi

# Rust: check for hardcoded secrets
echo "[CHECK] Scanning for hardcoded secrets in Rust files..."
grep -rn --include='*.rs' -E '(password|secret|api_key|token)\s*[:=]\s*"[^"]+"' . 2>/dev/null | grep -v '.agent/' | grep -v 'target/' | head -5 || true
...
grep -rn --include='*.ts' --include='*.tsx' --include='*.js' --include='*.jsx' \
  ...
  . 2>/dev/null | grep -v 'node_modules' | grep -v '.agent/' | grep -v '.d.ts' | head -5 || true
...
# Check for known secret patterns in all files
...
SECRET_HITS=$(grep -rn \
  ...
  . 2>/dev/null | grep -v 'node_modules' | grep -v '.agent/' | grep -v '.git/' | grep -v 'target/' | head -5 || true)
